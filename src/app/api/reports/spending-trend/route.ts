...
export async function GET(req: NextRequest) {
  return agentRoute(async (req) => {
    ...
    const rows = await createReportsService(getDb()).spendingTrend(userId, parsed.data.months, allowlist, req.nextUrl.searchParams.get("includeExcluded") === "1", req.nextUrl.searchParams.get("includePending") !== "0");
    return ok({ rows });
