...
export async function GET(req: NextRequest) {
  return agentRoute(async (req) => {
    ...
    const rows = await createReportsService(getDb()).spendingByCategory(userId, parsed.data.from, parsed.data.to, allowlist, req.nextUrl.searchParams.get("includeExcluded") === "1", req.nextUrl.searchParams.get("includePending") !== "0");
    return ok({ rows });
